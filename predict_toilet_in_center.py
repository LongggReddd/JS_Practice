...
def find_bath_rectangle(c_n_result):
    i =0
    bath_rectangle=[]
    for i in range(len(c_n_result)):      
        if '浴廁' in (c_n_result) [i][-1]:
            bath=((c_n_result)[i][:-1])
...
def center_point(test):
    ...
    contour, test = detect.detectOuterContours(gray, blank_image, color=(255,0,0))
    c = contour
    mv = cv2.moments(c)
...
img = cv2.imread("detection_testing/Images/49.jpg")
...
bath_rectangle = find_bath_rectangle(c_n_result)

center_point = center_point(test)

# ------------------------------------------------------------
# Search center point
paint_center_point = cv2.circle(test,center_point,10,(0,0,255),-1,0)
